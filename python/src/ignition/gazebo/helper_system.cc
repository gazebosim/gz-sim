...
define_gazebo_helper_fixture(py::object module)
{
  ...
  .def(
    "server", &HelperFixture::Server, "server pointer"
  )
  .def(
    "finalize", &HelperFixture::Finalize, "Finalize"
  )
  .def(
    "on_pre_update", &HelperFixture::OnPreUpdate, "OnPreUpdate"
  )
  .def(
    "on_update", &HelperFixture::OnUpdate, "OnUpdate"
  )
  .def(
    "on_post_update", &HelperFixture::OnPostUpdate, "OnPostUpdate"
  )
  .def(
    "on_configure", &HelperFixture::OnConfigure, "on_configure"
  );
}
}
}
