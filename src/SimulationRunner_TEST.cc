...
TEST_P(SimulationRunnerTest, StopBeforeRun)
{
  ...
  // Bound the wait at 10s (100 x 100ms) so a regression fails fast instead of
  // hanging the suite.
  ...
  // If the stop request was lost the runner is spinning in the run loop; a
  // second stop lets it exit so join() returns and the test reports the
  // EXPECT failure above instead of hanging.
  if (!returned)
    runner.Stop();
  runThread.join();

  // The stop must have prevented any stepping at all, and must be reported to
  // the caller rather than silently returning success.
