...
/// \brief Test topic name resolution for NavSat system
      ...
      const std::string &_expectedTopicName)
{
  ...
  auto plugin = std::make_unique<NavSat>();
  UpdateInfo info;
  info.paused = true;
...
TEST(NavSatTest, AbsoluteTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
      <model name="navsat_model" namespace="ns">
        <link name="link">
...
TEST(NavSatTest, RelativeTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
