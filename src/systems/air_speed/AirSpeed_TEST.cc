...
/// \brief Test topic name resolution for AirSpeed system
      ...
      const std::string &_expectedTopicName)
{
  ...
  auto plugin = std::make_unique<AirSpeed>();
...
TEST(AirSpeedTest, AbsoluteTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
      <model name="air_speed_model" namespace="ns">
        <link name="link">
...
TEST(AirSpeedTest, RelativeTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
