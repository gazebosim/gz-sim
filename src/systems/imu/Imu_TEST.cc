...
/// \brief Test topic name resolution for Imu system
      ...
      const std::string &_expectedTopicName)
{
  ...
  auto plugin = std::make_unique<Imu>();
  UpdateInfo info;
  info.paused = true;
...
TEST(ImuTest, AbsoluteTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
      <model name="imu_model" namespace="ns">
        <link name="link">
...
TEST(ImuTest, RelativeTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
