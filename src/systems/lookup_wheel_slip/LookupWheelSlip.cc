...
class gz::sim::systems::LookupWheelSlipPrivate
{
  ...
  public: static constexpr int kLateralColorChannel = 0;

  /// \brief Longitudal slip color channel representation (green channel)
  public: static constexpr int kLongitudalColorChannel = 1;

  /// \brief Friction color channel representation (blue channel)
  public: static constexpr int kFrictionColorChannel = 2;

  /// \brief Nominal slip color representation
  public: static constexpr int kNominalColor = 128;
};
    ...
    const EntityComponentManager &_ecm)
{
  ...
  {
    std::cerr << "Receievd nominal param: " << coeffs.first
    ...
    const EntityComponentManager &_ecm)
{
  for (const auto &linkEnt : this->linkEntities)
  {
    ...
    math::Color color = this->slipMapImg.Pixel(u, v);
    int latDeltaScale =
        static_cast<int>(color[kLateralColorChannel]) - kNominalColor;
    ...
    int lonDeltaScale =
        static_cast<int>(color[kLongitudalColorChannel]) - kNominalColor;
    ...
    int muDeltaScale =
        static_cast<int>(color[kFrictionColorChannel]) - kNominalColor;
