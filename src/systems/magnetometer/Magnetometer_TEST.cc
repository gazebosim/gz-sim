...
#include "Magnetometer.cc"

using namespace gz;
using namespace sim;
using namespace systems;

/// \brief Test topic name resolution for Magnetometer system
      ...
      const std::string &_expectedTopicName)
{
  ...
  auto plugin = std::make_unique<Magnetometer>();
  UpdateInfo info;
  info.paused = true;
...
TEST(MagnetometerTest, AbsoluteTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
      <model name="magnetometer_model" namespace="ns">
        <link name="link">
...
TEST(MagnetometerTest, RelativeTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
