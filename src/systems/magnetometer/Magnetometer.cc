...
constexpr const int8_t declination_table[13][37] = \
{
...
constexpr const int8_t inclination_table[13][37] = \
{
...
constexpr const int8_t strength_table[13][37] = \
{
    ...
    [&](const Entity &_entity,
        ...
        const components::WorldPose *_worldPose)->bool
      {
        ...
        if (it != this->entitySensorMap.end())
        {
          ...
          float declination_rad = get_mag_declination(lat_rad * 180 / M_PI, lon_rad * 180 / M_PI) * M_PI / 180;
          float inclination_rad = get_mag_inclination(lat_rad * 180 / M_PI, lon_rad * 180 / M_PI) * M_PI / 180;

          // Magnetic strength (10^5xnanoTesla)
          float strength_ga = 0.01f * get_mag_strength(lat_rad * 180 / M_PI, lon_rad * 180 / M_PI);
