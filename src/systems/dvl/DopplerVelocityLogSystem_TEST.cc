...
#include "DopplerVelocityLogSystem.cc"
      ...
      const std::string &_expectedTopicName)
{
  ...
  auto plugin = std::make_unique<DopplerVelocityLogSystem>();
...
TEST(DopplerVelocityLogTest, AbsoluteTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
      <model name="dvl_model" namespace="ns">
        <link name="link">
...
TEST(DopplerVelocityLogTest, RelativeTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
