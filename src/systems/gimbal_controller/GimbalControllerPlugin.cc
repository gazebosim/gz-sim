    ...
    EventManager &/*_eventMgr*/)
{
	...
	this->dataPtr->yawJoint = this->dataPtr->model.JointByName(_ecm, 
                                                 "yawJointName");
	...
	this->dataPtr->rollJoint = this->dataPtr->model.JointByName(_ecm, 
                                             "rollJointName");
	...
	if(_sdf->HasElement("joint_roll"))
	{
      ...
      this->dataPtr->model.JointByName(_ecm, "rollJointName");
		}
  ...
  this->dataPtr->model.JointByName(_ecm, "pitchJointName");
	...
	if(_sdf->HasElement("joint_pitch"))
	{
		...
		{
			this->dataPtr->pitchJoint = 
      this->dataPtr->model.JointByName(_ecm, "picthJointName");
		}
