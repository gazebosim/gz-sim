...
#include "LogicalCamera.cc"

using namespace gz;
using namespace sim;
using namespace systems;

/// \brief Test topic name resolution for LogicalCamera system
      ...
      const std::string &_expectedTopicName)
{
  ...
  auto plugin = std::make_unique<LogicalCamera>();
  UpdateInfo info;
  info.paused = true;
...
TEST(LogicalCameraTest, AbsoluteTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
      <model name="logical_camera_model" namespace="ns">
        <link name="link">
...
TEST(LogicalCameraTest, RelativeTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
