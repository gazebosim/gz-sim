                                              ...
                                              gz::msgs::Result &_res)
{
  ...
  switch (_req.command())
  {
    case gz::msgs::AttachDetachRequest::ATTACH:
    {
      ...
      _res.set_error_code(0);
      _res.set_message("Attach request accepted for child model [" + this->childModelName +
                       "] at link [" + this->childLinkName + "].");
      return true;
    }

    case gz::msgs::AttachDetachRequest::DETACH:
    {
      // If detach is requested
      if (!this->isAttached)
      {
        _res.set_error_code(1);
        _res.set_message("Detach request rejected: no child link is currently attached.");
    ...
    case gz::msgs::AttachDetachRequest::COMMAND_UNSPECIFIED:
    default:
    {
      _res.set_error_code(1);
