...
/// \brief Test topic name resolution for EnvironmentalSensorSystem
      ...
      const std::string &_expectedTopicName)
{
  ...
  auto plugin = std::make_unique<EnvironmentalSensorSystem>();
  UpdateInfo info;
  info.paused = true;
...
TEST(EnvironmentalSensorSystemTest, AbsoluteTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
      <model name="env_sensor_model" namespace="ns">
        <link name="link">
          <sensor name="env_sensor" type="custom"
                  gz:type="environmental_sensor/humidity">
...
TEST(EnvironmentalSensorSystemTest, RelativeTopicName)
{
  const std::string sdfString = R"(
  <sdf version="1.10">
    <world name="default">
