...
TEST(QuaternionTest, ConstructEuler)
{
  ...
  // There is an infinite number of equivalent Euler angle
  // representations when pitch = PI/2, so rather than comparing Euler
  // angles, we will compare quaternions.
  for (double roll = 0; roll < 2 * IGN_PI + 0.1; roll += IGN_PI/4) {
    for (double yaw = 0; yaw < 2 * IGN_PI + 0.1; yaw += IGN_PI/4) {
      math::Quaterniond q_orig(roll, IGN_PI/2, yaw);
      math::Quaterniond q_derived(q_orig.Euler());
      EXPECT_TRUE(q_orig == q_derived);
    }
  }
}
