...
namespace ignition
{
  namespace gazebo
  {
    ...
    class IGNITION_GAZEBO_VISIBLE Server
    {
      ...
      public: std::optional<std::reference_wrapper<EntityComponentManager>> SharedEntityComponentManager(
        const unsigned int _worldIndex = 0);

      /// \brief Get the Event Manager reference
      /// \param[in] _worldIndex Index of the world in the simrunner
      /// \return The Event Manager reference
      public: std::optional<std::reference_wrapper<EventManager>> SharedEventManager(
        const unsigned int _worldIndex = 0);
