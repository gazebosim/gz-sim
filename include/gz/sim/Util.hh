...
namespace gz
{
  namespace sim
  {
    ...
    math::Pose3d GZ_GAZEBO_VISIBLE worldPose(const Entity &_entity,
    ...
    /// entity name
    std::string GZ_GAZEBO_VISIBLE scopedName(const Entity &_entity,
    ...
    std::unordered_set<Entity> GZ_GAZEBO_VISIBLE entitiesFromScopedName(
    ...
    ComponentTypeId GZ_GAZEBO_VISIBLE entityTypeId(const Entity &_entity,
        const EntityComponentManager &_ecm);
    ...
    /// \param[in] _ecm Immutable reference to ECM.
    /// \return ID of entity's type-defining components.
    std::string GZ_GAZEBO_VISIBLE entityTypeStr(const Entity &_entity,
        const EntityComponentManager &_ecm);
    ...
    Entity GZ_GAZEBO_VISIBLE worldEntity(const Entity &_entity,
        const EntityComponentManager &_ecm);

    /// \brief Get the first world entity that's found.
    /// \param[in] _ecm Immutable reference to ECM.
    /// \return World entity ID.
    Entity GZ_GAZEBO_VISIBLE worldEntity(
        const EntityComponentManager &_ecm);
    ...
    std::string GZ_GAZEBO_VISIBLE removeParentScope(
    ...
    std::string GZ_GAZEBO_VISIBLE asFullPath(const std::string &_uri,
    ...
    gz::sim::Entity GZ_GAZEBO_VISIBLE topLevelModel(
    ...
    std::string GZ_GAZEBO_VISIBLE resolveSdfWorldFile(
    ...
    std::optional<math::Vector3d> GZ_GAZEBO_VISIBLE sphericalCoordinates(
