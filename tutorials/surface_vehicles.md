...
Then, let's build our workspace:
...
The main chasis is captured with the two pontoons is captured by `base_link`.
...
rotate. This is an additional degree of freedom that we allow to make the
thrusters more flexible. Then, the `left_engine_link` is connected to the
...
Normally, there's a section here where we talk about adding buoyancy to our
...
* [Optional] `<vehicle_length>` is the length of the vessel [m].
...
* [Optional] `<points>` contains a collection of points where the forces
             ...
             point next:
* [Optional] `<point><position>` Relative position of the point relative to
             `link_name`.
* [Optional] `<wavefield>`: The wavefield parameters. See `Wavefield.hh`.
...
```bash
gz topic -t /model/wam-V/joint/right_engine_propeller_joint/cmd_thrust -m gz.msgs.Double -p 'data: -0.15'
```
