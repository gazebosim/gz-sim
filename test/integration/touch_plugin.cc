...
TEST_F(TouchPluginTest, GZ_UTILS_TEST_DISABLED_ON_WIN32(AutoCreateContactSensor))
{
  ...
  // There are 3 touch plugins in the green box, and each one specifies a
  // different collision. The green box does not have any contact sensors in
  // its link but because the <create_contact_sensor_for_collision> is set
  // to true for the first 2 touch plugins, the plugins should auto create
  // the contact sensors for the green box. The last touch plugin sets the
  // parameter to false so it should not trigger any events because that no
  // contact sensors are available.
  // Drop the white box over one of the collisions in the
  // green box and verify that the correct touch event is published. Repeat
  // and drop the white box onto the other collision.
  std::string whiteBox = R"EOF(
  ...
  <sdf version="1.9">
      <model name="green_box_for_white">
        ...
        <plugin
          ...
          <namespace>white_touches_green_collision_01</namespace>
          <enabled>true</enabled>
          <create_contact_sensor_for_collision>true</create_contact_sensor_for_collision>
        </plugin>
        <plugin
          filename="gz-sim-touchplugin-system"
          name="gz::sim::systems::TouchPlugin">
        ...
        <plugin
          ...
          <namespace>white_touches_green_collision_03</namespace>
          <enabled>true</enabled>
          <create_contact_sensor_for_collision>false</create_contact_sensor_for_collision>
        </plugin>
      </model>
  </sdf>)EOF";
  ...
  // Verify that the /white_touches_green_collision_03/touched service is not
  // available because the contact sensor collision, collision_03, does not
  // exist and it should not be auto-created
  std::vector<std::string> serviceList;
  node.ServiceList(serviceList);
  EXPECT_EQ(std::find(serviceList.begin(), serviceList.end(),
                      "/white_touches_green_collision_03/touched"),
