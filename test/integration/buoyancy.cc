...
// collision origin: exact values for an origin offset mesh and the same
// mesh scaled.
TEST_F(BuoyancyTest, IGN_UTILS_TEST_DISABLED_ON_WIN32(MeshCenterOfVolume))
{
...
TEST_F(BuoyancyTest, IGN_UTILS_TEST_DISABLED_ON_WIN32(GradedSdfParsingOrder))
{
  ServerConfig serverConfig;
  const auto sdfFile = common::joinPaths(std::string(PROJECT_SOURCE_PATH),
...
TEST_F(BuoyancyTest, IGN_UTILS_TEST_DISABLED_ON_WIN32(GradedDivideByZero))
{
  ServerConfig serverConfig;
  const auto sdfFile = common::joinPaths(std::string(PROJECT_SOURCE_PATH),
