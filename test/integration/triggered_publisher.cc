...
TEST_F(TriggeredPublisherTest,
       IGN_UTILS_TEST_DISABLED_ON_WIN32(TEST))
{
  transport::Node node;
  auto inputPub = node.Advertise<msgs::Empty>("/in_20");
  std::atomic<std::size_t> recvCount{0};
  std::atomic<std::size_t> recvSrvCount{0};

  auto msgCb = std::function<void(const msgs::StringMsg &)>(
      [&recvCount](const auto & msg)
      {
        ignerr <<"recieved msg is [" << msg.data() << "]"<< std::endl;
        ++recvCount;
      });
  node.Subscribe("/out_20", msgCb);

  auto srvEchoCb = std::function<bool(const msgs::StringMsg &,
      msgs::Boolean &)>(
      [&recvSrvCount](const auto &req, auto &)
        {
          EXPECT_EQ(req.data(), "test");
          if (req.data() == "test")
          {
  ...
  waitUntil(3000, [&]{return pubCount == recvCount;});
  EXPECT_EQ(recvCount, recvCount);
  waitUntil(3000, [&]{return pubCount == recvSrvCount;});
  EXPECT_EQ(recvCount, recvSrvCount);
  // The simulation delay is 1000ms, which is equal to 1000 steps. Run
  // for 999 steps, and the count should still be zero. Take one additional
  // step and all the messages should arrive.
  this->server->Run(true, 4999, false);
  waitUntil(5000, [&]{return pubCount == recvCount;});
  EXPECT_EQ(0u, recvCount);
  this->server->Run(true, 1, false);
  waitUntil(5000, [&]{return pubCount == recvCount;});
