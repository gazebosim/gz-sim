...
class FourwidsDriveTest : public InternalFixture<::testing::Test>
{
                                 ...
                                 const std::string &_odomTopic)
  {
                          ...
                          double yaw_component)
    {
      ...
      EXPECT_NEAR(poses[1020].Pos().X(), odomPoses[0].Pos().X(), 3e-2);
      EXPECT_NEAR(poses[1020].Pos().Y(), odomPoses[0].Pos().Y(), 3e-2);
      EXPECT_NEAR(poses.back().Pos().X(), finalModelFramePose.Pos().X(), 3e-2);
      EXPECT_NEAR(poses.back().Pos().Y(), finalModelFramePose.Pos().Y(), 3e-2);
    };
