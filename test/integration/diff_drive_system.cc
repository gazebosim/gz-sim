...
class DiffDriveTest : public InternalFixture<::testing::TestWithParam<int>>
{
  ...
  /// \param[in] _cmdVelTopic Command velocity topic.
  /// \param[in] _odomTopic Odometry topic.
  /// \param[in] _enableTopic Enable topic.
  /// \param[in] _odomPubFreq Odometry publish frequency.
  protected: void TestEnableDisableCmd(const std::string &_sdfFile,
                                 const std::string &_cmdVelTopic,
                                 const std::string &_odomTopic,
                                 const std::string &_enableTopic,
                                 const double _odomPubFreq)
  {
    // Start server
    ServerConfig serverConfig;
    serverConfig.SetSdfFile(_sdfFile);
    ...
    // Publish command and check that vehicle moved
    double period = 1.0 / _odomPubFreq;
    double lastMsgTime{1.0};
    std::vector<math::Pose3d> odomPoses;
    std::function<void(const msgs::Odometry &)> odomCb =
      [&](const msgs::Odometry &_msg)
    ...
    // Disable controller
    auto pub_enable = node.Advertise<msgs::Boolean>(_enableTopic);
    ...
    int sleep = 0;
    int maxSleep = 70;
    const auto expectedOdomCount = static_cast<size_t>(7 * _odomPubFreq);
    for (; odomPoses.size() < expectedOdomCount && sleep < maxSleep; ++sleep)
    {
      std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    ...
    // Odom for 7s
    ASSERT_FALSE(odomPoses.empty());
    EXPECT_EQ(expectedOdomCount, odomPoses.size());
