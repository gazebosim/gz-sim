...
class WebsocketServerTest : public InternalFixture<::testing::Test>
{
  ...
  // Build a minimal `<plugin>` SDF element by parsing a real world string;
  // this gives the element proper value descriptors so Configure()'s
  // elem->Get<T>() calls work. `_extraChildren` is inserted verbatim under
  // the plugin tag.
  protected: std::shared_ptr<const sdf::Element> MakePluginSdf(
      const std::string &_extraChildren = "")
  {
    std::ostringstream xml;
    xml << "<?xml version='1.0'?>\n"
        << "<sdf version='1.11'>\n"
        << "  <world name='ws_test'>\n"
        << "    <plugin filename='gz-sim-websocket-server-system'\n"
        << "            name='gz::sim::systems::WebsocketServer'>\n"
        // Port 0 lets the OS pick an ephemeral port so concurrent test
        // runs do not collide. We bind to loopback by default; address
        // tests override <address> via _extraChildren.
        << "      <port>0</port>\n"
    ...
    sdf::init(sdfParsed);
    if (!sdf::readString(xml.str(), sdfParsed))
    {
      ADD_FAILURE() << "sdf::readString failed:\n" << xml.str();
      return nullptr;
    }

    auto worldElem = sdfParsed->Root()->GetElement("world");
    return worldElem->GetElement("plugin");
  }
  ...
  // Friend access is granted to this fixture class only; TEST_F bodies
  // live in generated subclasses and cannot directly touch private
  // members. Expose targeted accessors for the address-validation tests.
  protected: static const std::string &Address(
      const sim::systems::WebsocketServer &_server)
  {
    return _server.address;
  }

  // Drive Configure() with a freshly-built plugin SDF. Returns the
  // sanitized address that Configure() decided to use, or "" if SDF parse
  // failed (ADD_FAILURE is already recorded by MakePluginSdf in that case).
  protected: std::string ConfigureAndReadAddress(
      sim::systems::WebsocketServer &_server,
      const std::string &_extraChildren = "")
  {
    auto pluginElem = this->MakePluginSdf(_extraChildren);
    if (!pluginElem)
      return "";
    _server.Configure({}, pluginElem, this->ecm, this->eventMgr);
    return _server.address;
  }
  ...
  // Configure() requires non-const references; declare per-test instances
  // so each TEST_F gets a fresh pair. EventManager is non-copyable.
  protected: sim::EntityComponentManager ecm;
  protected: sim::EventManager eventMgr;
};
...
// -------------------------------------------------------------------------
// Configure: address/iface sanitization regression tests.
// Each test exercises Configure(), which actually opens a libwebsockets
// context on the configured (sanitized) address. We use port=0 to let
// the OS assign a free port. The destructor joins the service thread.
// -------------------------------------------------------------------------

TEST_F(WebsocketServerTest, Address_DefaultsToLoopback)
{
  sim::systems::WebsocketServer server;
  EXPECT_EQ("127.0.0.1", this->ConfigureAndReadAddress(server));
}

TEST_F(WebsocketServerTest, Address_AcceptsValidIPv4)
{
  sim::systems::WebsocketServer server;
  EXPECT_EQ("127.0.0.1", this->ConfigureAndReadAddress(server,
...
TEST_F(WebsocketServerTest, Address_RejectsShellInjection)
{
  sim::systems::WebsocketServer server;
  EXPECT_EQ("127.0.0.1", this->ConfigureAndReadAddress(server,
      "<address>; rm -rf /</address>\n"))
      << "shell-style metacharacters in <address> bypassed sanitization";
}

TEST_F(WebsocketServerTest, Address_RejectsTooLong)
{
  sim::systems::WebsocketServer server;
  EXPECT_EQ("127.0.0.1", this->ConfigureAndReadAddress(server,
      "<address>" + std::string(300, 'a') + "</address>\n"))
      << "overlong <address> bypassed the 255-char limit";
}

TEST_F(WebsocketServerTest, Address_AcceptsBindAll)
{
  sim::systems::WebsocketServer server;
  // '*' is the documented bind-all sentinel; it must survive the
  // character whitelist via the explicit special-case.
  EXPECT_EQ("*", this->ConfigureAndReadAddress(server,
      "<address>*</address>\n"));
}

TEST_F(WebsocketServerTest, Address_IfaceFallbackWhenAddressMissing)
{
  sim::systems::WebsocketServer server;
  // No <address>, only <iface>; same sanitization must apply.
  EXPECT_EQ("127.0.0.1", this->ConfigureAndReadAddress(server,
      "<iface>127.0.0.1</iface>\n"));
}
